...
var renderDownloadForms = function(err, data) {
    ...
    keys = _.map(data.rows, function(row) {
        ...
        return utils.localizedString(objectpath.get(def, 'meta.label') || 'Unknown');
    });
    keys = _.unique(keys);
    keys.sort();
    ...
    var forms = _.reduce(data.rows, function(memo, row) {
        ...
        title = utils.localizedString(objectpath.get(def, 'meta.label') || 'Unknown')
...
exports.sms_forms = function (doc, req) {
    ...
    events.once('afterResponse', function() {
        ...
        setupContext(req, function(err) {
            ...
            utils.updateTopNav('sms-forms-data', 'Forms');
...
exports.config = function(doc, req) {
    var settings =  doc.app_settings|| {};
    var response = settings.facility_labels || {};
